...
def validate_file_path(
    ...
    if len(path) > 255:
        raise ValueError("File path is too long (max 255 characters).")
    ...
    try:
        ...
        absolute_path = os.path.realpath(os.path.abspath(path))
         ...
         raise ValueError(f"Invalid file path structure: {e}")
    ...
    if base_dir:
        ...
        try:
            ...
            if common != resolved_base:
             ...
             raise ValueError("Access denied: Path is on a different drive.")
    ...
    if allowed_extensions:
        ...
        if ext.lower() not in valid_exts:
            raise ValueError(f"Invalid file extension '{ext}'. Allowed: {', '.join(valid_exts)}")
